...
from MyApp.forms import AppointmentForm, PatientUserInfo, PatientSignUpForm, DoctorUserInfo, DoctorSignUpForm, AdminUserInfo, AdminSignUpForm, PatientAppointmentForm
...
#-----------------------------------------------------------------------------------
@login_required(login_url='MyApp:patientlogin')
def patient_dashboard(request):
    appointments = Appointment.objects.all().filter(patientId=request.user.id).order_by('-id')[:1]
    appointment_count = Appointment.objects.all().filter(patientId=request.user.id).count()
    
    # Initialize mydict with default values
    mydict = {
...
def patient_my_appointment(request):
    ...
    print(appointments,request.user.id)
...
def admin_discharge_patient(request):
    patients = Patient.objects.filter(assignedDoctorId__isnull=False)
...
def discharge_patient(request, pk):
    ...
    assignedDoctor = get_object_or_404(Doctor, user_id=patient.assignedDoctorId)
    ...
    if request.method == 'POST':
        try:
            ...
            total_cost = (roomCharge * days_spent) + doctorFee + medicineCost + otherCharge

            feeDict = {
                'roomCharge': roomCharge * days_spent,
                'doctorFee': doctorFee,
            ...
            # Save discharge details to the database
            ...
            error_message = "Please ensure all fields are filled in correctly and are valid numbers."
            return render(request, 'MyApp/patient_generate_bill.html', context={**patientDict, 'error_message': error_message})
...
def admin_add_appointment(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            doctor_name=User.objects.get(id=doctor_id)
            patient_name=User.objects.get(id=patient_id)
            appointment_date = form.cleaned_data['appointmentDate']
            time = form.cleaned_data['time']
            description = form.cleaned_data['description']

            print(patient_id, doctor_id,doctor_name,patient_name, appointment_date, time, description)
            ...
            appointment = Appointment(
                ...
                status=True  # Assuming appointments are active by default
            )
            appointment.save()
            return render(request, 'MyApp/admin_appointment.html')
    else:
    ...
    return render(request, 'MyApp/admin_add_appointment.html', {'appointmentForm': form})
